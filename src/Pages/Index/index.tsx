...
const Index = () => {
    ...
    const renderTools = (): ReactElement[] => {
        ...
        Tools.forEach(tool => {
            const newTool: ToolsType = {title: tool.title, icon: tool.icon, list: []};
            if (searchText != undefined) {
                tool.list.forEach(listItem => {
                    if (listItem.name.indexOf(searchText) > -1) {
                        newTool.list.push({name: listItem.name, path: listItem.path, component: listItem.component});
                    }
                })
            } else {
        ...
        return newList.map(item => {
            return (
                <Container key={item.title} title={item.title} icon={item.icon} list={item.list}/>
            );
        })
    }
